...
\item each $\vec x_i$ consist of $S$'s variables of the same type as $S_i$'s input
\item each $\vec y_i$ consist of $S$'s local/output variables of the same type as $S_i$'s output
\item the subsystem directed graph rooted at $S$ is acyclic 
\end{enumerate}
}
