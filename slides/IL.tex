...
All \alert{infinite} traces that satisfy the \alert{FO-LTL} formula 
\[
\begin{array}{c@{~~}c@{~}c}
 \underbrace{
  I[\vec i,\vec o,\vec s] \land 
  P[\vec i,\vec o,\vec s]
 }_{} 
 & \land ~~\textbf{always}
 & (\underbrace{
  T[\vec i,\vec o,\vec s,\vec i',\vec o',\vec s'] \land 
  P[\vec i',\vec o',\vec s']
 }_{})
 \\[-1ex]
 \alert{I_S} & & \alert{T_S}
...
~~\outputa ((out Int)) \\
~~\inita (=> clock (= out in)) \comm{; output is arbitrary when clock is off}\\
...
\begin{frame}{System \alert{definition} command \qquad \deemph{(Composite case)}}
...
~~\outputa ((g1 Bool) (g2 Bool))
  \locala ((b1 Bool) (b2 Bool)) \\
~~\inva (and \\
~~~~(NonDetArbiter r1 r2 g1 g2)\\
~~~~\comm{; If there have been no requests, there have been no grants} \\
~~~~(= b1 (=> (Historically (and (not r1) (not r2))) \\
~~~~~~~~~~~~~~(Historically (and (not g1) (not g2))))) \\
~~~~\comm{; A request can be granted at most 4 times} \\
~~~~(= b2 (=> (Before (= (Count g1) 4)) (not g1)))))
\\[1.4ex]
(\checksys Monitor
...
\uncover<2,9>{\empha{Other types are useful!}}
\smallskip

\item
Stronger syntactic restrictions for \inita and \transa formulas \\[.5ex]
\uncover<3,9>{\empha{Should be enforced in the user-facing language}}
\smallskip

\item
Direct support for LTL, or your favorite temporal logic, in \checksys \\[.5ex]
\uncover<4,9>{\empha{Generality, mostly}}
\smallskip

\item
Global (mutable) variables \emph{a la} SAL \\[.5ex]
\uncover<5,9>{\empha{Tricky to get right}}
\smallskip

\item
Parametric components as in SMV or SAL \\[.5ex] 
\uncover<6,9>{\empha{Some support. The rest is better provided in the user-facing language }}
\smallskip

\item
Compositional reasoning features
(i.e., assume-guarantee contracts) \\[.5ex]
\uncover<7,9>{\empha{Too many different approaches out there}}
\smallskip
...
\begin{frame}{\alert{Output format} for satisfied queries}
\small
...
(\definesys $B$
  ...
  \outputa (($o$ $\tau_B$))
  \locala (($s$ $\theta_B$)) \\
~~\inva ( $\cdots$ (A $\cdots$) $\cdots$) $\ldots$
)
\ \\[1.2ex]
(\checksys B ~$\cdots$~
 \querya ($q_1$ $\cdots$) $\cdots$  \querya ($q_n$ $\cdots$) )\\
}
\bigskip
Output of \expr{\checksys}:
\medskip

\footnotesize
\expr{%
(($q_1$ 
     (\comm{; SMT-LIB interpretation of free symbols} \\
~~~~~~$\cdots$ \\
~~~~~) \\ 
~~~~~(\comm{; trace prefix} \\
~~~~~~(0 (($i$ $v_{i,0}$) ($o$ $v_{o,0}$) ($s$ $v_{s,0}$)
              ($A$::$i$ $v_{i,0}$) ($A$::$o$ $v_{o,0}$) ($A$::$s$ $v_{s,0}$))) \\
~~~~~~$\cdots$ \\
~~~~~) \\
~~~~~(\comm{; lasso} \\
~~~~~~($k$ (($i$ $v_{i,1}$) ($o_1$ $v_{o,1}$) ($s_1$ $v_{s,1}$)
              ($A$::$i_1$ $v_{i,1}$) ($A$::$o_1$ $v_{o,1}$) ($A$::$s_1$ $v_{s,1}$))) \\
~~~~~~$\cdots$ \\
~~~~~) \\
~) \\
~$\cdots$ \\
